...
1. resize the image to input_resize(input augment, default is 600).
	image shape: (1, depths, rows, cols, 1)
2. generate the region proposals match groundtruth best:
	y_rpn_cls: shape=(1, featuremap_depth, featuremap_height, featuremap_width, anchor_nums*2)
				Contains each anchor box [is_valid_for_use, is_positive] on each point on the feature map.

	y_rpn_regr: shape=(1, featuremap_depth, featuremap_height, featuremap_width, 2*4*anchor_nums)
				Then last dimension related to: (is_positive,is_positive,is_positive,is_positive, tx1, tx2, tx3, tr) latter 4 are the offset between propsoal and groundtruth
...
def get_new_img_size(width, height, depth, img_min_side=600):
        assert depth == height
        assert height == width
					...
					for kz in range(output_dense):
						...
						for bbox_num in range(num_bboxes):
							...
							if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > C.rpn_max_overlap:
								...
								tx1 = (cx1 - cx1a) / (r_anc*2.0)
								tx2 = (cx2 - cx2a) / (r_anc*2.0)
								tx3 = (cx3 - cx3a) / (r_anc*2.0)

								tr = np.log((gta[bbox_num,3]*2.0) / (r_anc*2.0))
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				x_img = resize(x_img, (resized_dense, resized_height, resized_width))
				...
				if backend == 'channels_last':
					x_img = np.transpose(x_img, (0, 2, 3, 4, 1)) #For RGB image: (1, cols, rows, 3)
