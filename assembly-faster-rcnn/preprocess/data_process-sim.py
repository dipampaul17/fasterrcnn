...
def data_extra(fname,f,f_res,model='train'):
    ...
    #file=np.load('cut/sim-data'+fname+'.npy')
    file=np.load('cut/'+fname, allow_pickle=True)
    num = len(file)
    ...
    for file_no in range(num):
        ...
        for roi_no in range(num_roi):
            ...
            print(int(bbox[2])+cut_z,int(math.ceil(bbox[5]))-cut_z)
            if model=='train':
                if not(os.path.exists('sim-images')):
                    os.mkdir('sim-images')
        ...
        if model=='test':    
            if not(os.path.exists('test-sim-images')):
                    os.mkdir('test-sim-images')
...
if __name__ == '__main__':
    ...
    #ids = ['00e6d0f7-08a2-43c9-976e-c8972dcf4bf3', ]
    ids = os.listdir('cut')
