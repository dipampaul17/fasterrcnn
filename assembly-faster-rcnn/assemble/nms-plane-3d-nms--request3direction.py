...
def nms_for_plane(dic, overlap_thresh=0.9, dis_threshold=10, max_boxes=300):
    ...
    while len(idxs) > 0:
        ...
        if len(np.where(overlap > overlap_thresh)[0]) > 0: 
            target_ids = list(np.where(overlap > overlap_thresh)[0])
            if len(target_ids)!= len(set(target_ids)):
                print 'warning, target_ids has duplicate items'
            for iii in target_ids:
                ind = inds[idxs[iii]]
                if abs(ind - c_ind) > dis_threshold:
                    #exceed distance, should be another one
                    target_ids.remove(iii)
                else:
                    rec_ind.append(ind)
            if len(target_ids)>0:
...
def cube_fprepare(pred, gt, f):
    ...
    for gt_box in gt:
        if gt_box['class'] not in f_pred:
            f_pred[gt_box['class']] = []
            f_gt[gt_box['class']] = []
        f_gt[gt_box['class']].append(1)
        if gt_box['bbox_matched']:
            f_pred[gt_box['class']].append(1)
        else:
            f_pred[gt_box['class']].append(0)

    for pred_box in pred:
        if pred_box['class'] not in f_pred:
            f_pred[pred_box['class']] = []
            f_gt[pred_box['class']] = []

        if not pred_box['bbox_matched']:
            f_gt[pred_box['class']].append(0)
            f_pred[pred_box['class']].append(1)

    out_pred = []
    out_gt = []
    for name in f_gt.keys():
        if int(float(name)) == 0:
            out_gt.extend([i * 13 for i in f_gt[name]])
            out_pred.extend([i * 13 for i in f_pred[name]])
        else:
            out_gt.extend([i * int(float(name)) for i in f_gt[name]])
            out_pred.extend([i * int(float(name)) for i in f_pred[name]])
...
if __name__ == '__main__':
    ...
    for tomo_id in predict_result:
        if 'f' not in predict_result[tomo_id]:
            predict_result[tomo_id]['f']={'fxx':[0.106,0.106], 'fyy':[0.106,0.106], 'fzz':[0.106,0.106]}
        fgt,fpred = cube_fprepare(predict_result[tomo_id]['bbox'], gt_dic[tomo_id]['bbox'], predict_result[tomo_id]['f'])
        FF_gt.extend(fgt)
        FF_pred.extend(fpred)
        f = f1_score(fgt,fpred, average='macro')
